...
public class UpdateUserProfile : Profile
{
    public UpdateUserProfile()
    {
        ...
        CreateMap<UpdateUserResult, UpdateUserResponse>()
            .ForMember(dest => dest.Name, opt => opt.MapFrom(src => new UpdateUserNameRequest
            {
                Firstname = src.Firstname,
                Lastname = src.Lastname
            }))
            .ForMember(dest => dest.Address, opt => opt.MapFrom(src => new UpdateUserAddressRequest
            {
                City = src.City,
                Street = src.Street,
                Number = src.AddressNumber,
                Zipcode = src.Zipcode,
                Geolocation = new UpdateUserGeolocationRequest { Lat = src.Lat, Long = src.Long }
            }));
    }
}
